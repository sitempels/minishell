...
int								env_updateone(t_env **head, char *key,
									char *value);
...
t_node							*node_addback(t_node *node, t_node *new,
									int mode);
...
char							*expand_exit_code(char *res, int i,
									int exit_status);
char							*expand_variable(char *input, size_t *i,
									t_env *env);
...
char							*get_path(t_shell *shell, char *cmd, t_env *env,
									int mode);
/**/
...
int								ft_error(t_shell *shell, int quit,
									int nbr_error, ...);
